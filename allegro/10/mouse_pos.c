    ...
    while (running)
    {
        ...
        if (redraw)
        {
            ...
            if (points_stored == 2)
            {
                ...
                al_draw_text(font, al_map_rgb(255, 255, 255), 20,
                             20 + id * 30, 0, buffer);

                id = 3;
                snprintf(buffer, sizeof(buffer), "Modulo   %d: u = %.2f km",
                         id + 1,
                         calcular_modulo_vetor(pos_km_x[0], pos_km_y[0],
                                               pos_km_x[1], pos_km_y[1])
