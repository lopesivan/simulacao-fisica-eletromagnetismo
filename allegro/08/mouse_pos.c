...
int main()
{
    ...
    while (running)
    {
        ...
        case ALLEGRO_EVENT_KEY_DOWN:
            ...
            else if (ev.keyboard.keycode == ALLEGRO_KEY_SPACE)
            {
                if (points_stored < 2)
                {
                     ...
                     * seja, isso aqui:
                    armazenar_posicao_mouse(mouse_x, mouse_y, screen_width,
                                            screen_height, &pos_km_x[0],
                                            &pos_km_y[0], points_stored);
                     */
        ...
        if (redraw)
        {
            ...
            {
                converter_km_para_pixel(pos_km_x[i], pos_km_y[i],
                                        screen_width, screen_height, &px,
                                        &py);

                al_draw_filled_circle(px, py, 10, al_map_rgb(255, 0, 0));

                snprintf(buffer, sizeof(buffer),
                         "Posição %d: X = %.2f km, Y = %.2f km", i + 1,
                         pos_km_x[i], pos_km_y[i]);

                /* DEBUG: imprime buffer em stderr */
                fprintf(stderr, "TRACE: %s\n", buffer);

                al_draw_text(font, al_map_rgb(255, 255, 255), 20, 20 + i * 30,
                             0, buffer);
            }
            ...
            snprintf(buffer, sizeof(buffer),
                     "Delta   %d: dX = %.2f km, dY = %.2f km", id + 1,
                     pos_km_x[1] - pos_km_x[0], pos_km_y[1] - pos_km_y[0]);

            /* DEBUG: imprime buffer em stdout */
            printf("TRACE: %s\n", buffer);

            al_draw_text(font, al_map_rgb(255, 255, 255), 20, 20 + id * 30, 0,
                         buffer);
